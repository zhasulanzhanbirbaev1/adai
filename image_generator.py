...
async def suggest_offers(niche: str, city: str = "Казахстан") -> dict:
    """AI предлагает 3 оффера, специфичных именно для этой ниши."""
    if not client:
        return {"offers": []}
    response = await client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": f"""Ты таргетолог, который знает казахстанский малый бизнес изнутри.

Ниша: {niche}
Город: {city}

Придумай 3 РАЗНЫХ рекламных оффера именно для этой ниши.

ЖЁСТКИЕ ТРЕБОВАНИЯ:
1. Назови КОНКРЕТНУЮ услугу или товар из этой ниши — не «услуги», не «сервис», не «товары».
   Для стоматологии это виниры, чистка, имплант. Для автосервиса — замена масла, развал-схождение.
   Для кофейни — завтрак, обед, кофе навынос. Думай, что реально продаёт этот бизнес.
2. В каждом оффере ОБЯЗАТЕЛЬНА цифра. Разные типы:
   - цена: «Виниры от 45 000 ₸»
   - срок: «Новая улыбка за 3 визита»
   - время: «Замена масла за 20 минут»
   - условие: «Рассрочка 0% на 12 месяцев»
   - подарок: «Чистка в подарок»
3. Цены — реальные для Казахстана в этой нише. Не выдумывай абсурдных сумм.
4. Три оффера должны отличаться ТИПОМ цифры, а не только услугой.
5. Длина — 4–9 слов. Это строка, которую владелец бизнеса вставит в рекламу.
6. Запрещено: «качество», «индивидуальный подход», «лучшие», «широкий спектр», «доступные цены».
...
{{"offers": ["оффер 1", "оффер 2", "оффер 3"]}}"""}],
    ...
    return {"offers": [str(o).strip() for o in data.get("offers", []) if str(o).strip()][:3]}
